...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // ⚡ Bolt Optimization: Filter base array before expensive mapping operations
  ...
  if (filters.zona) baseAnnunci = baseAnnunci.filter(a => {
    const restUser = db.getUser(a.ristoranteId);
    return restUser && restUser.zona === filters.zona;
  });
