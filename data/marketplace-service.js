...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // ⚡ Performance Optimization: Filter raw fields before expensive .map() operations
  let rawItems = db.annunci.filter(a => a.stato === 'aperto');
  if (filters.zona) rawItems = rawItems.filter(a => db.getUser(a.ristoranteId).zona === filters.zona);
  if (filters.tipo) rawItems = rawItems.filter(a => cleanText(a.tipo) === filters.tipo);
  if (filters.budget) rawItems = rawItems.filter(a => (a.budget || 0) >= parseInt(filters.budget, 10));
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  if (filters.esperienza) rawWorkers = rawWorkers.filter(w => (w.esperienza || 0) >= parseInt(filters.esperienza, 10));
  if (filters.budget) rawWorkers = rawWorkers.filter(w => (w.pagaMin || 0) <= parseInt(filters.budget, 10));
