...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  for (const annuncio of db.annunci) {
    ...
    // Apply filters early before expensive operations
    if (filters.tipo && cleanText(annuncio.tipo) !== filters.tipo) continue;
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  for (const worker of db.getCamerieri()) {
    // Basic filters first before expensive getEnhancedWorker
    if (filters.esperienza && worker.esperienza < parseInt(filters.esperienza, 10)) continue;
    if (filters.budget && worker.pagaMin > parseInt(filters.budget, 10)) continue;
    if (filters.zona && worker.zona !== filters.zona) continue;

    const profile = getEnhancedWorker(worker.id);

    // Skill filter requires hardSkills/qualifiche
