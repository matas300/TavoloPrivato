...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // We apply base filters (budget, zona) *before* the expensive .map() operation.
  // This avoids running `getEnhancedRestaurant` and `buildPairMetrics` on items we'll discard.
  if (filters.budget) {
    const minBudget = parseInt(filters.budget, 10);
    baseAnnunci = baseAnnunci.filter(a => a.budget >= minBudget);
  }

  if (filters.zona) {
    baseAnnunci = baseAnnunci.filter(a => {
      const rist = db.getUser(a.ristoranteId);
      return rist && rist.zona === filters.zona;
    });
  }
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // Bolt: Performance Optimization
  // We apply base filters (zona, esperienza, budget) *before* the expensive .map() operation.
  // This avoids running `getEnhancedWorker` and `buildPairMetrics` on items we'll discard.
  if (filters.zona) baseWorkers = baseWorkers.filter(w => w.zona === filters.zona);
  if (filters.esperienza) baseWorkers = baseWorkers.filter(w => w.esperienza >= parseInt(filters.esperienza, 10));
  if (filters.budget) baseWorkers = baseWorkers.filter(w => w.pagaMin <= parseInt(filters.budget, 10));
