...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  if (filters.esperienza) baseWorkers = baseWorkers.filter(worker => (worker.esperienza || 0) >= parseInt(filters.esperienza, 10));
  if (filters.budget) baseWorkers = baseWorkers.filter(worker => (worker.pagaMin || 0) <= parseInt(filters.budget, 10));
