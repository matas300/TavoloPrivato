...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // ⚡ Bolt: Apply basic filters before expensive mapping to avoid unnecessary work
  if (filters.tipo) baseAnnunci = baseAnnunci.filter(item => item.tipo === filters.tipo);
  if (filters.budget) baseAnnunci = baseAnnunci.filter(item => item.budget >= parseInt(filters.budget, 10));
