...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  const worker = getEnhancedWorker(workerId);
  // ⚡ Bolt Optimization: Replace map-filter chain with reduce to avoid redundant enhancement of discarded items
  const items = db.annunci.reduce((acc, annuncio) => {
    ...
    if (filters.budget && normalizedAnnuncio.budget < parseInt(filters.budget, 10)) return acc;
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  const workers = db.getCamerieri().reduce((acc, worker) => {
    ...
    if (filters.esperienza && profile.esperienza < parseInt(filters.esperienza, 10)) return acc;
    if (filters.budget && profile.pagaMin > parseInt(filters.budget, 10)) return acc;
