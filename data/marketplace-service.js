...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // Filter base arrays before mapping to avoid expensive enhancements and pair metrics on discarded items
  let baseItems = db.annunci.filter(a => a.stato === 'aperto');

  if (filters.tipo) baseItems = baseItems.filter(item => item.tipo === filters.tipo);
  if (filters.budget) baseItems = baseItems.filter(item => item.budget >= parseInt(filters.budget, 10));
  if (filters.zona) baseItems = baseItems.filter(item => db.getUser(item.ristoranteId).zona === filters.zona);
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // PRE-FILTERING OPTIMIZATION:
  // Filter base arrays before mapping to avoid expensive `getEnhancedWorker` and `buildPairMetrics` calls on discarded items
