...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // ⚡ Bolt Optimization: Filter raw base data before expensive enhancements
  let baseAnnunci = db.annunci.filter(a => a.stato === 'aperto');
  if (filters.budget) {
    const budgetVal = parseInt(filters.budget, 10);
    baseAnnunci = baseAnnunci.filter(a => a.budget >= budgetVal);
  }
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  if (filters.esperienza) {
    ...
    baseWorkers = baseWorkers.filter(w => w.esperienza >= expVal);
  }
  if (filters.budget) {
    const budgetVal = parseInt(filters.budget, 10);
    baseWorkers = baseWorkers.filter(w => w.pagaMin <= budgetVal);
  }
