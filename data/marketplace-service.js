...
// Optimization: Accept preFilteredContracts to avoid redundant O(N) filtering inside loops
...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // Optimization: Pre-group contracts by ristoranteId for O(1) lookup
  const contractsByRestaurant = new Map();
  for (const c of db.contratti) {
    if (c.cameriereId === workerId) {
      if (!contractsByRestaurant.has(c.ristoranteId)) {
        contractsByRestaurant.set(c.ristoranteId, []);
      }
      contractsByRestaurant.get(c.ristoranteId).push(c);
    }
  }
  let items = db.annunci.filter(a => a.stato === 'aperto').map(annuncio => {
    ...
    const restaurantContracts = contractsByRestaurant.get(annuncio.ristoranteId) || [];
    const compliance = buildPairMetrics(workerId, annuncio.ristoranteId, restaurantContracts);
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // Optimization: Pre-group contracts by cameriereId to avoid O(N²) .filter() inside the map loop
  const contractsByWorker = new Map();
  for (const c of db.contratti) {
    if (c.ristoranteId === restaurantId) {
      if (!contractsByWorker.has(c.cameriereId)) {
        contractsByWorker.set(c.cameriereId, []);
      }
      contractsByWorker.get(c.cameriereId).push(c);
    }
  }
  let workers = db.getCamerieri().map(worker => {
    const profile = getEnhancedWorker(worker.id);
    const workerContracts = contractsByWorker.get(worker.id) || [];
    const compliance = buildPairMetrics(worker.id, restaurantId, workerContracts);
    return {
      ...profile,
      compliance,
      complianceUi: decisionUi(compliance.decision),
...
function getRestaurantDashboard(restaurantId) {
  ...
  // Optimization: Pre-group contracts by worker to avoid O(N²) filtering inside loops
  const contractsByWorker = new Map();
  ...
  const upcoming = contratti.filter(c => c.stato === 'confermato').map(c => ({
    ...
    compliance: buildPairMetrics(c.cameriereId, restaurantId, contractsByWorker.get(c.cameriereId) || [])
  }));
  const completed = contratti.filter(c => ['pagato', 'completato'].includes(c.stato));
  ...
  const roster = [...new Set(contratti.map(c => c.cameriereId))].map(workerId => {
    const worker = getEnhancedWorker(workerId);
    const workerContracts = contractsByWorker.get(workerId) || [];
