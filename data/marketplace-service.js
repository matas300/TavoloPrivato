...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  const worker = getEnhancedWorker(workerId);
  // Optimization: Single loop avoids redundant data normalization and skips
  // expensive operations (getEnhancedRestaurant, buildPairMetrics) for filtered items.
  const items = [];
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  const restaurant = getEnhancedRestaurant(restaurantId);
  // Optimization: Single loop applies filters before performing expensive
  // calculations like buildPairMetrics and matchScore.
  const workers = [];
