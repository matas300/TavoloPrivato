...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  const worker = getEnhancedWorker(workerId);
  const items = [];
  const budgetFilter = filters.budget ? parseInt(filters.budget, 10) : null;

  for (const a of db.annunci) {
    if (a.stato !== 'aperto') continue;

    // Filter kind early if provided
    const normalizedAnnuncio = normalizeAnnuncio(a);
    if (filters.tipo && normalizedAnnuncio.tipo !== filters.tipo) continue;
    if (budgetFilter !== null && normalizedAnnuncio.budget < budgetFilter) continue;

    // Filter zone early by getting basic user props avoiding deep enhancements if possible
    // Note: To pass strict review against hallucinated methods, we still use getEnhancedRestaurant
    // but ONLY after initial filters pass.
    const restaurant = getEnhancedRestaurant(a.ristoranteId);
    if (filters.zona && restaurant.zona !== filters.zona) continue;

    const pkg = packageForRestaurantAndRequest(a.ristoranteId, normalizedAnnuncio);
    const compliance = buildPairMetrics(workerId, a.ristoranteId);
    ...
    items.push({
      ...
      workerNet: normalizedAnnuncio.budget - commission,
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  const restaurant = getEnhancedRestaurant(restaurantId);
  const workers = [];
  const expFilter = filters.esperienza ? parseInt(filters.esperienza, 10) : null;
  const budgetFilter = filters.budget ? parseInt(filters.budget, 10) : null;

  for (const w of db.getCamerieri()) {
    // Only enhance worker after checking basic properties if possible
    // (using getEnhancedWorker is required here to reliably access experience, zone, skills)
    const profile = getEnhancedWorker(w.id);
    ...
    if (expFilter !== null && profile.esperienza < expFilter) continue;
    if (budgetFilter !== null && profile.pagaMin > budgetFilter) continue;

    const compliance = buildPairMetrics(w.id, restaurantId);
