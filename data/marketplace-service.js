...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // ⚡ Bolt: filter raw base arrays *before* the expensive O(N) mapping operations
  // to avoid computing enhancements for items that will just be dropped.
  let rawItems = db.annunci.filter(a => {
    if (a.stato !== 'aperto') return false;
    if (filters.tipo && cleanText(a.tipo) !== filters.tipo) return false;
    if (filters.budget && (a.budget || 0) < parseInt(filters.budget, 10)) return false;
    if (filters.zona) {
      const rest = db.getUser(a.ristoranteId);
      if (!rest || rest.zona !== filters.zona) return false;
    }
    return true;
  });
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // ⚡ Bolt: filter raw base arrays *before* expensive map (getEnhancedWorker is costly)
  // `qualifica` is kept after map because it depends on computed `hardSkills`.
  let rawWorkers = db.getCamerieri().filter(w => {
    if (filters.zona && w.zona !== filters.zona) return false;
    if (filters.esperienza && (w.esperienza || 0) < parseInt(filters.esperienza, 10)) return false;
    if (filters.budget && (w.pagaMin || 0) > parseInt(filters.budget, 10)) return false;
    return true;
  });
