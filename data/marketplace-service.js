...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  // ⚡ Bolt Optimization: Avoid chained map/filter to skip expensive computations (like getEnhancedRestaurant, buildPairMetrics) for items that get filtered out.
  const worker = getEnhancedWorker(workerId);
  let items = [];
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  // ⚡ Bolt Optimization: Avoid chained map/filter to skip expensive computations (like buildPairMetrics) for items that get filtered out.
  const restaurant = getEnhancedRestaurant(restaurantId);
  let workers = [];
