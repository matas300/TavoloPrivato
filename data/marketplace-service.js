...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  const worker = getEnhancedWorker(workerId);
  // ⚡ Bolt: Pre-group contracts to avoid O(N^2) array filtering in loop
  const contractsByRestaurant = db.contratti
    .filter(c => c.cameriereId === workerId)
    .reduce((acc, c) => {
  ...
  let items = db.annunci.filter(a => a.stato === 'aperto').map(annuncio => {
    ...
    const restContracts = contractsByRestaurant[annuncio.ristoranteId] || [];
    const compliance = buildPairMetrics(workerId, annuncio.ristoranteId, restContracts);
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  const restaurant = getEnhancedRestaurant(restaurantId);
  // ⚡ Bolt: Pre-group contracts to avoid O(N^2) array filtering in loop
  const contractsByWorker = db.contratti
    .filter(c => c.ristoranteId === restaurantId)
    .reduce((acc, c) => {
...
function getRestaurantDashboard(restaurantId) {
  ...
  // ⚡ Bolt: Pre-group contracts to avoid O(N^2) array filtering in loop
  const contractsByWorkerRoster = contratti.reduce((acc, c) => {
    if (!acc[c.cameriereId]) acc[c.cameriereId] = [];
    acc[c.cameriereId].push(c);
    return acc;
  }, {});
  ...
  const upcoming = contratti.filter(c => c.stato === 'confermato').map(c => ({
    ...
    compliance: buildPairMetrics(c.cameriereId, restaurantId, contractsByWorkerRoster[c.cameriereId] || [])
  }));
  const completed = contratti.filter(c => ['pagato', 'completato'].includes(c.stato));
  ...
  const roster = [...new Set(contratti.map(c => c.cameriereId))].map(workerId => {
    const worker = getEnhancedWorker(workerId);
    const metrics = buildPairMetrics(workerId, restaurantId, contractsByWorkerRoster[workerId] || []);
    return {
      ...worker,
      turniCount: (contractsByWorkerRoster[workerId] || []).length,
      compliance: metrics,
...
function getComplianceOverview(alerts = null) {
  alerts = alerts || getAdminAlerts();
  return {
...
function getAdminDashboard() {
  ...
  return {
    ...
    alerts: alerts,
    complianceOverview: getComplianceOverview(alerts)
  };
}
