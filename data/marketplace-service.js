...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // Single pass loop with early filter returns
  // avoids executing expensive enhancements and buildPairMetrics for filtered out requests
  for (const annuncio of db.annunci) {
    if (annuncio.stato !== 'aperto') continue;
    if (filters.tipo && annuncio.tipo !== filters.tipo) continue;
    if (minBudget !== null && annuncio.budget < minBudget) continue;
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // Single pass loop with early filter returns
  // avoids executing expensive buildPairMetrics and scoreWorkerToRestaurant for filtered out workers
