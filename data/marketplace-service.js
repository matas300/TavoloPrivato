...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  const worker = getEnhancedWorker(workerId);
  const minBudget = filters.budget ? parseInt(filters.budget, 10) : null;

  // ⚡ Bolt: Use single pass reduce and apply cheap filters first to avoid expensive normalizations on discarded items
  const items = db.annunci.reduce((acc, annuncio) => {
    if (annuncio.stato !== 'aperto') return acc;

    const normalizedAnnuncio = normalizeAnnuncio(annuncio);
    if (filters.tipo && normalizedAnnuncio.tipo !== filters.tipo) return acc;
    if (minBudget !== null && normalizedAnnuncio.budget < minBudget) return acc;

    const restaurant = getEnhancedRestaurant(annuncio.ristoranteId);
    if (filters.zona && restaurant.zona !== filters.zona) return acc;
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  const restaurant = getEnhancedRestaurant(restaurantId);
  const minEsperienza = filters.esperienza ? parseInt(filters.esperienza, 10) : null;
  const maxBudget = filters.budget ? parseInt(filters.budget, 10) : null;

  // ⚡ Bolt: Use single pass reduce and apply cheap filters first to avoid expensive metrics calculation on discarded workers
  const workers = db.getCamerieri().reduce((acc, worker) => {
    const profile = getEnhancedWorker(worker.id);

    if (filters.zona && profile.zona !== filters.zona) return acc;
    if (filters.qualifica && !profile.qualifiche.includes(filters.qualifica) && !profile.hardSkills.includes(filters.qualifica)) return acc;
    if (minEsperienza !== null && profile.esperienza < minEsperienza) return acc;
    if (maxBudget !== null && profile.pagaMin > maxBudget) return acc;
