...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // OPTIMIZATION: Filter raw fields before expensive map operations
  if (filters.zona) baseAnnunci = baseAnnunci.filter(a => db.getUser(a.ristoranteId)?.zona === filters.zona);
  if (filters.tipo) baseAnnunci = baseAnnunci.filter(a => cleanText(a.tipo) === filters.tipo);
  if (filters.budget) {
    const budgetVal = parseInt(filters.budget, 10);
    baseAnnunci = baseAnnunci.filter(a => (a.budget || 0) >= budgetVal);
  }
  ...
  // Note: Raw filters (zona, tipo, budget) are now applied before mapping
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  if (filters.esperienza) {
    const expVal = parseInt(filters.esperienza, 10);
    baseWorkers = baseWorkers.filter(w => (w.esperienza || 0) >= expVal);
  }
  if (filters.budget) {
    const budgetVal = parseInt(filters.budget, 10);
    baseWorkers = baseWorkers.filter(w => (w.pagaMin || 0) <= budgetVal);
  }
  ...
  // OPTIMIZATION: Keep derived filters (qualifica) after map
