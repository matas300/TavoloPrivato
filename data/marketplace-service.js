...
// Merged map/filter chains into a single loop.
// Previously, all items were mapped with expensive functions (getEnhancedRestaurant, buildPairMetrics)
// and then filtered out later. Now we skip mapping entirely for items that don't match the filters.
// This reduces unnecessary object creation and function calls, speeding up execution.
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  for (const annuncio of db.annunci) {
    ...
    if (minBudget && annuncio.budget < minBudget) continue;
...
// ⚡ Bolt Performance Optimization:
// Merged map/filter chains into a single loop.
// Skips creating enhanced profiles for workers that don't pass the initial filters.
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  for (const worker of db.getCamerieri()) {
    if (minEsperienza && worker.esperienza < minEsperienza) continue;
    if (maxBudget && worker.pagaMin > maxBudget) continue;
    if (filters.zona && worker.zona !== filters.zona) continue;
