...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // ⚡ Bolt: Apply basic filters before expensive mapping
  let rawAnnunci = db.annunci.filter(a => a.stato === 'aperto');
  if (filters.tipo) rawAnnunci = rawAnnunci.filter(a => cleanText(a.tipo) === filters.tipo);
  if (filters.budget) rawAnnunci = rawAnnunci.filter(a => a.budget >= parseInt(filters.budget, 10));
  if (filters.zona) rawAnnunci = rawAnnunci.filter(a => {
    const rBase = db.getUser(a.ristoranteId);
    return rBase && rBase.zona === filters.zona;
  });

  let items = rawAnnunci.map(annuncio => {
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  if (filters.zona) rawWorkers = rawWorkers.filter(w => w.zona === filters.zona);
  if (filters.esperienza) rawWorkers = rawWorkers.filter(w => w.esperienza >= parseInt(filters.esperienza, 10));
  if (filters.budget) rawWorkers = rawWorkers.filter(w => w.pagaMin <= parseInt(filters.budget, 10));
  ...
  // Qualifica uses hardSkills from profile, which requires mapping first
