...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // Pre-filter raw announcements before expensive map operations
  let rawAnnunci = db.annunci.filter(a => a.stato === 'aperto');
  if (filters.budget) {
    const minBudget = parseInt(filters.budget, 10);
    rawAnnunci = rawAnnunci.filter(a => a.budget >= minBudget);
  }
  if (filters.zona) {
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // Pre-filter raw workers before expensive map operations
  ...
  if (filters.esperienza) {
    ...
    rawWorkers = rawWorkers.filter(w => w.esperienza >= minExp);
  }
  if (filters.budget) {
    const maxBudget = parseInt(filters.budget, 10);
    rawWorkers = rawWorkers.filter(w => w.pagaMin <= maxBudget);
  }
