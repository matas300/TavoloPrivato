...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  const worker = getEnhancedWorker(workerId);
  // ⚡ Bolt: Early filtering on base data to avoid expensive mapping for items that will be discarded
  let items = db.annunci.filter(a => {
    if (a.stato !== 'aperto') return false;
    // budget is a raw field un-mutated by normalizers, safe to filter early
    if (filters.budget && !(a.budget >= parseInt(filters.budget, 10))) return false;
    return true;
  }).map(annuncio => {
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  const restaurant = getEnhancedRestaurant(restaurantId);
  // ⚡ Bolt: Apply basic, unchanged base property filters early to prevent expensive object enhancements
  let workers = db.getCamerieri().filter(worker => {
    if (filters.zona && worker.zona !== filters.zona) return false;
    if (filters.esperienza && !(worker.esperienza >= parseInt(filters.esperienza, 10))) return false;
    if (filters.budget && !(worker.pagaMin <= parseInt(filters.budget, 10))) return false;
    return true;
  }).map(worker => {
