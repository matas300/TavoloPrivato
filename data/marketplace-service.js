...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // O(N) pre-processing group worker contracts by restaurant to optimize buildPairMetrics
  ...
  let items = db.annunci.filter(a => a.stato === 'aperto').map(annuncio => {
    ...
    const compliance = buildPairMetrics(workerId, annuncio.ristoranteId, contractsByRestaurant.get(annuncio.ristoranteId) || []);
...
function getRestaurantDashboard(restaurantId) {
  ...
  // O(N) pre-processing group contracts by worker to avoid O(N^2) bottlenecks in map loops
  const workerContractsMap = new Map();
  for (const c of contratti) {
    if (!workerContractsMap.has(c.cameriereId)) {
      workerContractsMap.set(c.cameriereId, []);
    }
    workerContractsMap.get(c.cameriereId).push(c);
  }

  const upcoming = contratti.filter(c => c.stato === 'confermato').map(c => ({
    ...c,
    worker: getEnhancedWorker(c.cameriereId),
    compliance: buildPairMetrics(c.cameriereId, restaurantId, workerContractsMap.get(c.cameriereId) || [])
  }));
  const completed = contratti.filter(c => ['pagato', 'completato'].includes(c.stato));
  const monthSpend = completed.reduce((sum, c) => sum + c.compensoCam + c.commissione, 0);
  const roster = Array.from(workerContractsMap.keys()).map(workerId => {
    const workerContracts = workerContractsMap.get(workerId);
    const worker = getEnhancedWorker(workerId);
