...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // Early filter to avoid expensive operations
  ...
  if (filters.zona) {
    // Zona check needs restaurant's zone. We can look it up efficiently via db.getUser without full enhancement
    baseAnnunci = baseAnnunci.filter(a => {
      const restBase = db.getUser(a.ristoranteId);
      return restBase && restBase.zona === filters.zona;
    });
  }
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // qualifica filter requires hardSkills which are computed in getEnhancedWorker
