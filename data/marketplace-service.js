...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  for (const annuncio of db.annunci) {
    ...
    // ⚡ Bolt Optimization: Apply base filters before expensive operations
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  for (const baseWorker of db.getCamerieri()) {
    ...
    const bestPackage = restaurant.servicePackages.find(pkg => pkg.requiredSkills.some(skill => profile.qualifiche.includes(skill) || profile.hardSkills.includes(skill))) || restaurant.servicePackages[0];
