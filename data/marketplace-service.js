...
// Optimization: Use a single for...of loop to apply base filters before performing expensive normalizations and metric calculations
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  for (const annuncio of db.annunci) {
    ...
    // Apply base filters before expensive operations
    if (filters.budget && annuncio.budget < parseInt(filters.budget, 10)) continue;
...
// Optimization: Apply filters in a single pass to avoid redundant profile enhancements and compliance checks
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  const restaurant = getEnhancedRestaurant(restaurantId);
  const workers = [];

  for (const workerBase of db.getCamerieri()) {
    // Check base budget to avoid calculating the enhanced worker completely if it fails
    ...
    // Apply qualification base filter before expensive metric operations
    ...
    const compliance = buildPairMetrics(profile.id, restaurantId);
