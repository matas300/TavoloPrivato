...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  // Optimization: Use a single loop and apply filters before expensive metrics/scoring calculations
  const worker = getEnhancedWorker(workerId);
  const items = [];
  ...
  for (const annuncio of db.annunci) {
    ...
    if (minBudget !== null && normalizedAnnuncio.budget < minBudget) continue;
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  // Optimization: Replace map/filter chain with single loop to avoid calculating metrics for filtered items
  const restaurant = getEnhancedRestaurant(restaurantId);
  const workers = [];
  ...
  for (const baseWorker of db.getCamerieri()) {
    const profile = getEnhancedWorker(baseWorker.id);
    ...
    if (minEsperienza !== null && profile.esperienza < minEsperienza) continue;
    if (maxBudget !== null && profile.pagaMin > maxBudget) continue;

    const compliance = buildPairMetrics(baseWorker.id, restaurantId);
    workers.push({
      ...profile,
      compliance,
      complianceUi: decisionUi(compliance.decision),
