...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  for (const annuncio of db.annunci) {
    ...
    const restaurant = getEnhancedRestaurant(normalizedAnnuncio.ristoranteId);
    if (filters.zona && restaurant.zona !== filters.zona) continue;

    const pkg = packageForRestaurantAndRequest(normalizedAnnuncio.ristoranteId, normalizedAnnuncio);
    const compliance = buildPairMetrics(workerId, normalizedAnnuncio.ristoranteId);
