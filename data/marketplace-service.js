...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // ⚡ Bolt: filter base fields before expensive mappings
  let baseItems = db.annunci.filter(a => a.stato === 'aperto');
  if (filters.zona) {
  ...
  if (filters.tipo) baseItems = baseItems.filter(a => cleanText(a.tipo) === filters.tipo);
  if (filters.budget) baseItems = baseItems.filter(a => (a.budget || 0) >= parseInt(filters.budget, 10));

  let items = baseItems.map(annuncio => {
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // ⚡ Bolt: 'qualifica' depends on enhanced profile (hardSkills), leave after map
