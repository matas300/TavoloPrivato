...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // ⚡ Bolt: Apply budget and zona filters before expensive mappings
  let rawAnnunci = db.annunci.filter(a => a.stato === 'aperto');
  if (filters.budget) {
    const minBudget = parseInt(filters.budget, 10);
    rawAnnunci = rawAnnunci.filter(a => (a.budget || 0) >= minBudget);
  }
  if (filters.zona) {
    rawAnnunci = rawAnnunci.filter(a => db.getUser(a.ristoranteId)?.zona === filters.zona);
  }
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // ⚡ Bolt: Apply raw filters before expensive mappings
  let rawWorkers = db.getCamerieri();
  if (filters.zona) {
    rawWorkers = rawWorkers.filter(w => w.zona === filters.zona);
  }
  if (filters.esperienza) {
    const minEsperienza = parseInt(filters.esperienza, 10);
    rawWorkers = rawWorkers.filter(w => (w.esperienza || 0) >= minEsperienza);
  }
  if (filters.budget) {
    const maxPagaMin = parseInt(filters.budget, 10);
    rawWorkers = rawWorkers.filter(w => (w.pagaMin || 0) <= maxPagaMin);
  }
