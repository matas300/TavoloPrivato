...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // Apply raw filters before expensive mapping to improve performance
  let baseAnnunci = db.annunci.filter(a => a.stato === 'aperto');
  if (filters.tipo) baseAnnunci = baseAnnunci.filter(a => a.tipo === filters.tipo);
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  // Filter that depends on enhanced properties (hardSkills) remains here
