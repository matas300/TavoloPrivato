...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  // ⚡ Bolt: Filter base data before expensive mapping to prevent unnecessary processing
  let rawAnnunci = db.annunci.filter(a => a.stato === 'aperto');
  if (filters.tipo) {
    const rawTipo = cleanText(filters.tipo);
    rawAnnunci = rawAnnunci.filter(a => cleanText(a.tipo || '') === rawTipo);
  }
  if (filters.budget) {
    const budgetInt = parseInt(filters.budget, 10);
    rawAnnunci = rawAnnunci.filter(a => (a.budget || 0) >= budgetInt);
  }
  if (filters.zona) {
    rawAnnunci = rawAnnunci.filter(a => {
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  if (filters.esperienza) {
    const expInt = parseInt(filters.esperienza, 10);
    rawWorkers = rawWorkers.filter(w => (w.esperienza || 0) >= expInt);
  }
  if (filters.budget) {
    const budgetInt = parseInt(filters.budget, 10);
    rawWorkers = rawWorkers.filter(w => (w.pagaMin || 0) <= budgetInt);
  }
