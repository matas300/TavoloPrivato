...
function getOpenServiceRequestsForWorker(workerId, filters = {}) {
  ...
  for (const annuncio of db.annunci) {
    ...
    // Normalize once to apply base filters
    ...
    // Expensive operations (e.g., buildPairMetrics) only on items that pass filters
...
function getWorkerMatchesForRestaurant(restaurantId, filters = {}) {
  ...
  for (const rawWorker of db.getCamerieri()) {
    // Only apply getEnhancedWorker to normalize profile fields
    const profile = getEnhancedWorker(rawWorker.id);
    ...
    // Expensive operation (loops over contracts) only on filtered items
