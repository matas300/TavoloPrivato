...
## 2024-05-01 - Early array filtering before mapping
**Learning:** In `data/marketplace-service.js`, mapping over base data arrays with expensive in-memory operations (like `getEnhancedWorker` or `buildPairMetrics`) before applying filters causes unnecessary computations for records that are ultimately discarded.
**Action:** Always apply filters to base arrays using raw fields (like `budget`, `zona`, `esperienza`) *before* running expensive maps. Keep filters that rely on enhanced properties *after* the map.
