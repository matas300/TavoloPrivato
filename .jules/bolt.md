...
## 2025-02-13 - Array filtering before mapping
**Learning:** In heavily mock-data-driven apps, operations mapping over lists of objects to enhance them are extremely slow when unneeded elements aren't filtered out first. `getWorkerMatchesForRestaurant` and `getOpenServiceRequestsForWorker` were enhancing the entire array before applying basic filters like zone and budget.
**Action:** Always filter base arrays by simple properties (like budget, zone) BEFORE doing any heavy enhancements (mapping that hits other objects or complex calculations).
