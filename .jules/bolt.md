...
## 2025-01-24 - Pre-processing array groups to avoid O(N^2) `.filter` bottlenecks
**Learning:** Using `.filter()` inside loops like `.map()` (e.g. `roster.map(w => contratti.filter(...))`) causes an O(N^2) bottleneck. When processing large array datasets in memory mock backends, this scales poorly and slows down response times.
**Action:** Iterate the list once and group items into a `Map` structure before entering mapping loops. Then retrieve grouped records using `Map.get()` in O(1) time.
