...
## 2025-02-12 - Optimize loop filtering in marketplace-service.js
**Learning:** `getWorkerMatchesForRestaurant` and `getOpenServiceRequestsForWorker` in `data/marketplace-service.js` used a `.map().filter()` pattern, which forced expensive data formatting (`getEnhancedWorker`, `buildPairMetrics`) on ALL entries before filtering out the majority of them.
**Action:** Replaced `.map().filter()` with a single `for...of` loop with early continues. Applying basic filters *before* executing expensive enhancements reduces redundant object allocations and logic evaluation.
