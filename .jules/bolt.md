...
## 2025-02-23 - Optimize Array Map then Filter Pattern
**Learning:** Chaining `.map()` to build expensive objects and subsequently `.filter()` them creates major performance bottlenecks as expensive normalizations (like `buildPairMetrics`) are calculated for items discarded later.
**Action:** Use a single `for...of` loop or `.reduce()` to normalize items, apply base filters early, and only then perform expensive operations on the items that pass the filters.
