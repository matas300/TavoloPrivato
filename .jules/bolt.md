...
## 2025-02-13 - Optimizing list endpoints with pre-filtering
**Learning:** `getWorkerMatchesForRestaurant` and `getOpenServiceRequestsForWorker` in `marketplace-service.js` mapped over entire datasets performing expensive `getEnhancedWorker` and `normalizeAnnuncio` operations before filtering the results. This is inefficient as enhancements were being run on objects that were ultimately discarded.
**Action:** Move raw field filters (like `budget`, `zona`, `esperienza`, `tipo`) to run before the mapping operations. This prevents the execution of unnecessary in-memory enhancements. Performance on a mock database improved processing times significantly. Ensure to only pre-filter properties that are not modified during the enhancement process to maintain correct behavior.
