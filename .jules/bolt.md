...
## 2024-05-24 - Optimize filters in marketplace matching
**Learning:** Chaining `.filter().map()` operations causes significant performance loss when mapping functions are expensive (like `getEnhancedRestaurant`, `buildPairMetrics`).
**Action:** Use a single `for...of` loop and apply filters with `continue` statements *before* calling expensive enhancement functions to speed up marketplace listings filtering.
