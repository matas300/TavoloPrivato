...
## 2025-02-28 - Array .filter() in .map() O(N²) Bottleneck
**Learning:** The mock database was using `.filter()` on arrays containing thousands of items inside of a `.map()` loop (e.g. `db.contratti.filter(...)`), creating massive O(N²) performance bottlenecks.
**Action:** Pre-group the array elements into hash maps (e.g. using `.reduce()`) to perform O(N) pre-processing, allowing O(1) lookups inside loops instead.
