...
## 2024-05-24 - Data normalization and filtering execution order
**Learning:** In list rendering or filtering logic, running `.filter().map()` causes N+1 like performance problems by normalizing and deeply calculating (e.g., getting cross-referenced metrics) on rows that eventually get dropped.
**Action:** Apply base filters eagerly during a single loop `for...of` or `.reduce()` before continuing to expensive computations for rows that pass.
