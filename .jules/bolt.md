...
## 2025-02-18 - [Optimize array filtering]
**Learning:** Chaining `.filter()` and `.map()` on arrays can lead to expensive duplicate normalizations/enhancements in Node.js apps.
**Action:** When working with potentially large data sets where filtering depends on base properties, apply early filtering with `for...of` or `.reduce()` before applying expensive derivations.
