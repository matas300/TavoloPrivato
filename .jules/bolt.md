...
## 2025-02-12 - Pre-filter optimization in marketplace-service
**Learning:** Functions doing expensive enhancements/mapping (like `getEnhancedWorker` and `buildPairMetrics`) in large loops were taking too much CPU because they computed objects that would immediately be thrown away by a subsequent `.filter()` logic.
**Action:** Always filter base arrays before mapping when expensive transformations are involved. Pre-filtering the mock database arrays reduced API processing times from ~80-140ms down to ~20ms in targeted endpoints.
