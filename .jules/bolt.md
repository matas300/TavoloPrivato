...
## 2025-02-12 - Early returns to prevent expensive operations
**Learning:** In `data/marketplace-service.js`, iterating over large arrays using `.map()` followed by multiple `.filter()` calls caused a huge performance drain because heavy normalizations and metrics processing were done for every element even if it was going to be discarded.
**Action:** Replaced `.filter().map()` loops with a single `for...of` loop where filtering logic skips the element immediately, avoiding the expensive properties mapping/enhancement logic completely.
