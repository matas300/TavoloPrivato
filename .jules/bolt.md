...
## 2024-05-24 - Filter early to avoid N+1 and O(N*M) heavy computations
**Learning:** In list-fetching functions, running `.map()` with expensive data enrichment (like aggregating contracts for compliance scores) over the entire dataset before calling `.filter()` creates a huge performance bottleneck.
**Action:** Always apply basic filters early (e.g., using `for...of` loops) before performing expensive enrichment or database aggregations on the remaining items.
