...
## 2025-05-18 - Single Loop Over Array Methods
**Learning:** Chaining `.map()` and `.filter()` in JavaScript processes every item with expensive functions first, even if many are later discarded by the filter.
**Action:** Combine filtering and mapping logic into a single `for...of` loop to eagerly discard items and avoid redundant computation.
