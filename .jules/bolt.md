...
## 2025-05-31 - [Optimize map/filter chains]
**Learning:** Avoid chaining `.filter()` and `.map()` if it causes redundant data normalization (e.g., calling `normalizeAnnuncio` twice).
**Action:** Use a single `for...of` loop or `.reduce()` to normalize an item once, apply base filters, and only then perform expensive operations (like `getEnhancedRestaurant` or `buildPairMetrics`) on the items that pass the filters.
