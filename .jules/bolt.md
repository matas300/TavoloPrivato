...
## 2025-02-27 - O(N²) array filtering bottleneck in mock data
**Learning:** In the mock database architecture, repeated `.filter()` calls inside `.map()` loops (e.g. matching workers and restaurants) cause severe O(N²) performance bottlenecks because the filtering scans the entire contract array for every element.
**Action:** Always pre-group arrays into hash maps (O(N) pre-processing) before the loop for O(1) lookups inside the loop.
