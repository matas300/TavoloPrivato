...
## 2025-02-12 - Avoid map/filter chaining with expensive inner loops
**Learning:** Chaining `.map()` containing expensive operations (like data enrichment) followed by `.filter()` causes severe performance degradation, as expensive functions execute on items that will ultimately be discarded.
**Action:** Use a single `for...of` loop or `.reduce()` to filter early, ensuring expensive enhancement functions only run on items that pass the base criteria.
