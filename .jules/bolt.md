...
## 2023-10-27 - Pre-filter items before map enhancement operations
**Learning:** Found an N+1 scaling bottleneck in marketplace filters (`getWorkerMatchesForRestaurant` and `getOpenServiceRequestsForWorker`) where expensive data enhancements and normalization were applied to *all* dataset items before basic criteria (like budget or experience) were filtered out.
**Action:** Pre-filter base datasets (like `db.getCamerieri()` and `db.annunci`) on raw fields immediately to narrow down the array before doing expensive operations in `.map()`.
