...
## 2024-05-18 - Optimized Filtering in Data Service
**Learning:** When evaluating lists against expensive mapping functions (like `getEnhancedWorker` or `buildPairMetrics`), basic data filters must be applied *before* `.map()` to avoid processing data that will ultimately be discarded. In this mock-heavy structure, processing all items and then filtering led to unnecessary cycles.
**Action:** Before mapping records, pre-filter the source array using criteria that apply to base data shapes (e.g. `zona`, `budget`, `esperienza`, `tipo`).
