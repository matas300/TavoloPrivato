...
## 2024-05-18 - [Optimize Map/Filter Chaining in Marketplace Service]
**Learning:** Chaining `.map()` and `.filter()` on large arrays when the mapping function performs expensive normalizations or data enhancements (e.g., `normalizeAnnuncio`, `getEnhancedRestaurant`, `buildPairMetrics`) causes severe performance bottlenecks because all items are enriched before discarding non-matching ones.
**Action:** Always prefer a single `for...of` loop or `.reduce()` for marketplace queries to perform base filtering (e.g., matching basic properties like `budget`, `tipo`, `zona`) *before* executing expensive logic or calling deep profile enhancements on the item.
