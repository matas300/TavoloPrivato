...
## 2026-04-12 - Synchronous file operations
**Learning:** Multiple API endpoints in `routes/api.js` (`/blueprint/doc`, `/blueprint/legal-audit`, etc.) were using `fs.readFileSync` to serve static documentation files. This blocks the main Node.js event loop, crippling throughput for all concurrent users when these endpoints are accessed, especially under load.
**Action:** Replaced `res.send(fs.readFileSync(...))` with asynchronous `res.sendFile(...)`. This offloads the file I/O and unblocks the event loop, drastically improving server concurrency and responsiveness.
