...
## 2026-05-20 - Pre-mapping Array Filters
**Learning:** Found a performance bottleneck in `data/marketplace-service.js` where expensive operations (e.g. `getEnhancedWorker`, `buildPairMetrics`) were being run on arrays *before* filters were applied.
**Action:** When filtering base datasets, always apply raw field filters (`zona`, `budget`, etc.) before calling expensive `.map()` enhancements. Leave filters relying on mapped output (like derived `qualifica`) for after the map.
