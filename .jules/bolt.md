...
## 2025-02-12 - Early filtering optimization
**Learning:** In data processing pipelines involving complex enhancements (like `buildPairMetrics` which does multiple O(n) lookups), applying raw data filters before `.map()` operations dramatically reduces execution time.
**Action:** Move raw field filters (e.g. zona, budget, esperienza) before the map, but leave filters depending on derived fields (e.g. qualifica) after the map.
