...
## 2024-07-16 - O(N²) Bottleneck in Array Filtering Inside Loops
**Learning:** In the mock database, array `.filter()` inside `.map()` loops (e.g., filtering `db.contratti` per match) causes severe O(N²) performance bottlenecks.
**Action:** Optimize these by pre-grouping the arrays into hash maps (O(N) pre-processing) before the loop for O(1) lookups.
