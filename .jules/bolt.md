...
## 2025-02-12 - N+1 Array filtering optimization
**Learning:** In the mock database, array `.filter()` calls placed inside `.map()` loops (e.g., filtering `db.contratti` per match) causes severe O(N²) performance bottlenecks on large datasets.
**Action:** Optimize these bottlenecks by pre-grouping the arrays into hash maps (O(N) pre-processing) before the loop, enabling O(1) lookups inside the loop.
