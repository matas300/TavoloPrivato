...
## 2025-02-13 - Data pipeline map/filter loop optimization
**Learning:** Chaining `.filter().map()` over data sources when retrieving normalized worker or restaurant records leads to redundant instantiation and heavy operations (e.g., `buildPairMetrics`, `getEnhancedRestaurant`) on data that will eventually be discarded.
**Action:** Replace `.filter().map().filter()` chains with a single `for...of` loop where base scalar filtering is applied prior to computing expensive properties on candidate records.
