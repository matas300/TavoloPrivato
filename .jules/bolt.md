...
## 2024-05-24 - Optimize map/filter chains avoiding redundant computations
**Learning:** Chaining `.map()` and `.filter()` can lead to significant performance bottlenecks if the mapping phase includes expensive computations (e.g. nested lookups or building metrics) for items that are subsequently discarded in the filter phase.
**Action:** When filtering a dataset after computing expensive properties, use `.reduce()` (or a simple loop) to apply simple filters first, and lazily calculate the expensive properties only for the items that pass the filters.
