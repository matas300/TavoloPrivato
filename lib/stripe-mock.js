...
// Virtual event: non-Stripe-native, usa il canale StripeEvent per audit/DAC7.
// TavoloLibero è marketplace intermediario puro → il sostituto d'imposta è
// il ristorante (art. 23 DPR 600/73), non il marketplace. Questo evento
// REGISTRA la ritenuta trattenuta dal ristorante, ma nessun cash del
// marketplace passa dalla ritenuta. Il Sub-progetto 3 (webhook handler)
// consumerà l'evento per produrre il report F24 al ristorante.
const withholding = {
  async report(params, options) {
    return withIdempotency(options, async (idemKey) => {
      const payload = {
        ...
        restaurantProfileId: params.restaurantProfileId,
        amountEur: params.amountEur,
        rate: params.rate,
        taxRegime: params.taxRegime,
        sostitutoImposta: 'restaurant',
      };
      const providerEventId = await writeEvent('withholding.reported', payload, idemKey);
      return { id: providerEventId, object: 'withholding_report', eventType: 'withholding.reported', ...payload };
    });
  }
};
...
module.exports = { paymentIntents, invoices, transfers, accounts, customers, withholding, _resetForTests };
