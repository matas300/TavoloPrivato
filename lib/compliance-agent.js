...
function buildProjectedTotals(contracts, workerProfileId, restaurantProfileId, lookbackStart, proposal = null) {
  ...
  const pairGross24m = pairContracts.reduce((sum, contract) => sum + contract.taxableAmountEur, 0);
  const workerGross24m = workerContracts.reduce((sum, contract) => sum + contract.taxableAmountEur, 0);
  const pairDays24m = pairContracts.reduce((sum, contract) => sum + contractDays(contract), 0);

  const proposalGross = proposal?.taxableAmountEur || 0;
...
async function evaluatePairCompliance(prisma, options) {
  ...
  return {
    ...
    proposal: proposal
      ? {
        taxableAmountEur: Number((proposal.taxableAmountEur || 0).toFixed(2)),
