...
function buildContractSnapshot({ contractInput, workerProfile, restaurantProfile, milestones }) {
  const clauses = [
    ...
    },
    {
      ...
      body: `Il corrispettivo complessivo pattuito e EUR ${contractInput.taxableAmountEur.toFixed(2)}, con fee piattaforma inclusa e ripartizione economica dettagliata nel piano milestone allegato.`
    },
    {
...
async function createLongTermContract(prisma, rawInput) {
  ...
  const taxableAmountEur = money(rawInput.taxableAmountEur || 0);
  ...
  if (!taxableAmountEur && !monthlyGrossAmountEur) {
    throw new Error('Serve taxableAmountEur o monthlyGrossAmountEur per generare il contratto lungo.');
  }

  const milestones = buildMonthlyMilestones({
    startDate,
    endDate,
    taxableAmountEur: taxableAmountEur || null,
  ...
  const totalGrossAmountEur = milestones.reduce((sum, item) => sum + item.taxableAmountEur, 0);
  ...
  const compliance = await evaluatePairCompliance(prisma, {
    ...
    proposal: {
      taxableAmountEur: totalGrossAmountEur,
  ...
  for (const milestone of milestones) {
    const created = await prisma.contractMilestone.create({
      data: {
        ...
        taxableAmountEur: milestone.taxableAmountEur,
